...
.then(function(){ // click interview prepration kit
    ...
    let waitforSelector = page.waitforSelector("a[data-attr1='warmup']", {visible:true})
    let combinedPromise = Promise.all([clickPromise, page.waitForNavigation({waitUntil: "networkidle0"}), waitforSelector])
    return combinedPromise;
})
.then(function(){// click warmup challeneges
    let clickPromise = page.click("a[data-attr1='warmup']")
    let waitforSelector = page.waitforSelector("a[data-attr1='sock-merchant']", {visible:true})
    let combinedPromise = Promise.all([clickPromise, page.waitForNavigation({waitUntil: "networkidle0"}), waitforSelector])
    return combinedPromise;
})
